...
class RefreshWorkflow:
    ...
    def stage_2_build(self) -> bool:
        ...
        try:
            from pipeline.builder import build_database  # noqa: PLC0415
    ...
    def stage_5_enrich(self) -> bool:
        ...
        try:
            ...
            self.log(f"Enrichment complete in {fmt_time(elapsed).strip()}", "ok")
            ...
            self.results["enrich"] = "completed"
            self._write_progress("stage_5_enrich", "completed",
                                 f"{fmt_time(elapsed).strip()}, {tag_count} tags")
            return True
    ...
    def stage_3_validate(self) -> bool:
        ...
        try:
            from pipeline.validator import validate_all, print_report  # noqa: PLC0415
    ...
    def run(self, years: list[int], sources: list[str]) -> int:
        ...
        self.log(f"Total time: {fmt_time(elapsed).strip()}")
        self.log("")
        ...
        if success:
            ...
            self._write_progress("done", "failed",
                                 f"Workflow failed after {fmt_time(elapsed).strip()}")
