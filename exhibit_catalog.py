...
DONE 1.B1-a (OH-MY-006, 2026-02-19)  Cross-validated catalog against FY2026
    comptroller Excel corpus (7 files: p1, r1, o1, m1, p1r, rf1, c1).
    All 7 exhibits: 100% catalog field match against real headers.
    Key changes:
      - Replaced generic "Prior Year"/"Current Year"/"Budget Estimate" patterns
        with real FY-specific patterns ("Actuals", "Enacted", "Disc Request")
      - Added actual structural columns: Organization, BSA, AG/BSA, SAG/BLI,
        Line Number, Add/Non-Add, Include In TOA, Classification
      - C-1 completely revised (Construction Project, State/Country, etc.)
      - RF-1 revised to match O&M-style structure (not revenue/expense pairs)
      - P-1R revised as procurement reconciliation (not reserve type)
      - Fixed find_matching_columns() to use longest-match-first strategy
      - P-5, R-2, R-3, R-4 marked as NOT YET VERIFIED (J-Book format, not in corpus)

DONE 1.B1-b  EXHIBIT_CATALOG contains full column_spec for P-1, R-1, O-1, M-1
    with header_patterns verified against FY2026 real data.
...
DONE 1.B1-c  EXHIBIT_CATALOG contains full column_spec for P-5 (12 cols), R-2 (9),
    ...
    Note: these are J-Book exhibit types not yet verified against real corpus.

DONE 1.B1-d  C-1 column_spec verified against FY2026 real data: uses FY-prefixed
    Authorization Amount / Appropriation Amount columns, Construction Project
    hierarchy, State/Country location fields. RF-1 verified: uses O&M-style
    AG/BSA + SAG/BLI structure (not revenue/expense pairs as initially expected).

DONE 1.B1-e  All exhibit types in EXHIBIT_CATALOG include p1r (Procurement
    Reconciliation). J-Books and supplemental exhibits deferred.

DONE 1.B1-f  find_matching_columns() uses longest-match-first strategy;
...
# DONE 1.B1-a (OH-MY-006, 2026-02-19) — Cross-validated against FY2026 corpus.
#   All 7 comptroller exhibits matched. Catalog header_patterns updated.
#   See exhibit_catalog.py module docstring for full details.
