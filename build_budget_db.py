            ...
            try:
              for i, page in enumerate(pdf.pages):
                ...
                if _likely_has_tables(page):
                    ...
                    if issue_type:
                        # Record the issue for later analysis (BUILD-003: log timeouts)
                        conn.execute(
                        ...
                        if issue_type == "timeout":
                            print(
                                f"    TIMEOUT: {file_path.name} page {i+1} "
                                f"(>{pdf_timeout}s) — logged for triage"
                            )
                        page_issues_count += 1
                        tables = None
    ...
    try:
      if num_workers > 1 and len(pdfs_to_process) > 1:
        ...
        with ProcessPoolExecutor(max_workers=num_workers) as pool:
            future_to_pdf = {
                pool.submit(
                    _extract_pdf_data, (str(pdf), str(docs_dir), pdf_timeout)
                ): pdf
                for pdf in pdfs_to_process
            }
        ...
        for idx, pdf in enumerate(pdfs_to_process):
            ...
            pages = ingest_pdf_file(conn, pdf, page_callback=_page_cb,
                                    docs_dir=docs_dir, pdf_timeout=pdf_timeout)
            file_elapsed = time.time() - t0
